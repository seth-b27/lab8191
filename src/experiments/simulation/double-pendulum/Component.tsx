...
function getPositions(th1: number,  th2: number, pivotX: number, pivotY: number, cfg: PendulumConfig){
...
export default function DoublePendulum({config: configOverride, className, style}: DoublePendulumProps) {
    const cfg: PendulumConfig = useMemo(() => ({ ...DEFAULT_CONFIG, ...configOverride }), [configOverride]);
    ...
    const handleResize = useCallback(() => {
        ...
        // Reposition the static pivot to the center
        const px = w / 2;
        const py = h / 2;
    ...
    return (
        <svg
            ref={svgRef}
            className={className}
            style={{ backgroundColor: '#ECEBE7', ...style }}
