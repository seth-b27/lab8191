...
export async function generateStaticParams() {
    return typographyExperiments.map(e => ({ slug: e.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
    const { slug } = await params
    const exp = typographyExperiments.find(e => e.slug === slug)
    if (!exp) return {}
    return { title: exp.title }
}

export default async function TypographySlugPage({ params }: Props) {
    const { slug } = await params
    const exp = typographyExperiments.find(e => e.slug === slug)
    if (!exp) notFound()
    ...
    return (
        <>
            <CategoryLayout
                category="typography"
                experiments={typographyExperiments}
                activeSlug={slug}
