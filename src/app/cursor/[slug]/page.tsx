...
export async function generateStaticParams() {
    return cursorExperiments.map(e => ({ slug: e.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
    const { slug } = await params
    const exp = cursorExperiments.find(e => e.slug === slug)
    if (!exp) return {}
    return { title: exp.title }
}

export default async function CursorSlugPage({ params }: Props) {
    const { slug } = await params
    const exp = cursorExperiments.find(e => e.slug === slug)
    if (!exp) notFound()
